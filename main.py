...
# Описание бота — показывается в чате ДО нажатия Start (до 512 символов)
BOT_DESCRIPTION = (
    "Привет. Это твой трекер привычек.\n\n"
    "Он помогает держать ритм без насилия над собой.\n\n"
    "Добавь привычку и каждый день отмечай даже самый маленький шаг. Спокойно и регулярно.\n\n"
    "Нажми Start и сделай первый шаг сегодня."
)
...
ONBOARDING_PROMPT = (
    ...
    "Я буду рядом и помогу держать ритм."
)
...
async def process_add_habit_name(message: Message, state: FSMContext) -> None:
    ...
    if success:
        ...
        if count == 1:
            ...
            text = f"✅ Привычка «{habit_text}» добавлена!\n\nТеперь я буду спрашивать вас каждый день в 21:00 😊"
        else:
            text = f"✅ Привычка «{habit_text}» добавлена!\n\nЯ буду напоминать вам о ней каждый день в 21:00 по Москве 🌙"
        await message.answer(text, reply_markup=get_bot_menu(user_id))
    else:
...
async def main() -> None:
    ...
    try:
        ...
        logger.info("Описание бота обновлено (плашка до Start)")
    except Exception as e:
